...
stem_geneset<-read.delim('/geneset/stem_geneset.txt',stringsAsFactors = F,check.names = F)
...
LUAD_stem<-read.delim('/TCGA/stem_GSVA/LUAD/stem_GSVA.txt',stringsAsFactors = F,check.names = F)
...
LUAD_stem<-t(LUAD_stem) %>% as.data.frame()
write.table(LUAD_stem,'/TCGA/stem_GSVA/LUAD/stem_GSVA.txt',quote = F,sep = '\t')

LUAD_pheno<-read.delim('/TCGA/count/LUAD/TCGA-LUAD.GDC_phenotype.tsv',stringsAsFactors = F,check.names = F)
