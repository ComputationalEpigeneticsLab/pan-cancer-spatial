...
dir_re<-'E:/Mirror/ST_analysis/data/pic_data/bdy_step1_MP/bdy_step_1_coreMP_immInfiltration/immNormal/'
...
for(j in 1:5){#j=1
  ...
  pdf(paste0('E:/Mirror/ST_analysis/pic/NMF_module/coreBdyImm/',step_n[j],'_cir_celltype_MP_heatmap.pdf'),width = 6,height = 6)
  circos.clear()
  #circos.par(gap.degree=1)
  #circos.par(gap.after = c(rep(1,12), 10))
  circos.par(gap.after = c(rep(1,12),18))
  #?circos.par
  circos.heatmap(cir_data[,c(1:4,7:14)], col = plot_col, na.col = "white",####去除Endothelial  Epithelial
