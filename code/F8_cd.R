...
all_TCGA_count<-as.matrix(readRDS('/data/TCGA/allCancer/all_TCGA_count.rds'))
all_TCGA_pheno<-read.delim('/data/TCGA/allCancer/all_TCGA_pheno.txt',stringsAsFactors = F,check.names = F)
###去批次
library(sva)
library(tinyarray)
...
pdf('/data/TCGA/allCancer/before_T_N.pdf',height = 8,width=8)
draw_pca(exp = all_TCGA_count, group_list = factor(all_TCGA_pheno$T_N))
dev.off()

pdf('/data/TCGA/allCancer/before_cancer.pdf',height = 8,width=8)
...
pdf('/data/TCGA/allCancer/after_T_N.pdf',height = 8,width=8)
draw_pca(exp = expr_count_combat, group_list = factor(all_TCGA_pheno$T_N))
dev.off()

pdf('/data/TCGA/allCancer/after_cancer.pdf',height = 8,width=8)
...
saveRDS(sparsematrix_combat,file = '/data/TCGA/allCancer/all_TCGA_count_combat.rds')
...
# MP13<-read.delim('/data/TCGA/use_gene/MP_list_intersect_initia25_intersect_cluster24.txt',
#                  stringsAsFactors = F,check.names = F)
MP13<-MP13[,-1]
...
# CAF_marker<-read.delim('/data/TCGA/use_gene/CAF_marker.txt')
# TAM_marker<-read.delim('/data/TCGA/use_gene/TAM_marker.txt')
...
# bdy_step1CAF_divi<-read.delim('/data/TCGA/use_gene/bdy_step1CAF_diviLR.txt',stringsAsFactors=F,check.names=F)
# bdy_step1TAM_divi<-read.delim('/data/TCGA/use_gene/bdy_step1TAM_diviLR.txt',stringsAsFactors=F,check.names=F)

# matrix_CAF_divi<-read.delim('/data/TCGA/use_gene/matrix_CAF_diviLR.txt',stringsAsFactors = F,check.names = F)
# matrix_TAM_divi<-read.delim('/data/TCGA/use_gene/matrix_TAM_diviLR.txt',stringsAsFactors = F,check.names = F)
