...
######绘制比例条形图
dir_subTAM<-'/data/10X Visium/Cytospace/'
...
write.table(all_sliceTAM,'/data/pic_data/subTAM_all.txt',quote = F,sep = '\t',row.names = F)
all_sliceTAM<-read.delim('/data/pic_data/subTAM_all.txt',stringsAsFactors = F,check.names = F)
...
pdf('/pic/subCAFTAM/subTAM_bar_SPP1&FN1.pdf',height = 4,width = 6.5)
print(p_compare)
dev.off()
