...
dir_out<-'/data/10X Visium/EMT/'

dir_rds<-'/data/10X Visium/ST_expression/'
...
dir_bdy<-'/data/10X Visium/copykat/'
...
dir_RCTD<-'/data/10X Visium/RCTD3/'
...
write.table(all_step_Tcell,'/data/pic_data/all_step_TcellSI.txt',quote = F,sep = '\t',row.names = F)
all_step_Tcell<-read.delim('/data/pic_data/all_step_TcellSI.txt',stringsAsFactors = F,check.names = F)
...
########################
####按每个step画
#pdf('/pic/lihc_CAF_isolation/TcellSI/step_state_cluster.pdf',width = 4,height = 8)
pdf('/pic/lihc_CAF_isolation/TcellSI/step_state.pdf',width = 8,height = 3)
