...
for(i in 1:length(file_anno)){
  dir_out<-paste0("/10X Visium/inferCNV_R/",dataSet_name[i],"/")
  ...
  ###参考基因组
  gene_anno <- read.delim("/gencode.v43.annotation.txt",
...
###参考基因组 
geneFile <- read.table("/gencode.v43.annotation.txt",header = T,sep = "\t",stringsAsFactors = F)
geneFile<-geneFile[!duplicated(geneFile$gene_name),]
rownames(geneFile)=geneFile$gene_name


dir_bdy<-'/10X Visium/copykat/'
...
dir_CNV<-'/10X Visium/inferCNV_R2/'
