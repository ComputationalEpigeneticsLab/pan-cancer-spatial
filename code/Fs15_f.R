...
geneset <- clusterProfiler::read.gmt(gmtfile = '/data/geneset/h.all.v2025.1.Hs.symbols.gmt')

all_geneset<-lapply(unique(geneset$term),function(x) unique(geneset$gene[geneset$term%in%x]) )
names(all_geneset)<-unique(geneset$term)
...
ecosystem_cluster_maker<-read.delim('/10X Visium/new_st_NMF/eco_DE_Imm/core_eco_differ.txt',
                                    stringsAsFactors = F,check.names = F)
ecosystem_cluster_maker<-read.delim('/10X Visium/new_st_NMF/eco_DE_Imm/core_eco_6_differ.txt',
                                    stringsAsFactors = F,check.names = F)
...
GSEA_NESscore<-data.frame(pathway=names(all_geneset))
GSEA_FDR<-data.frame(pathway=names(all_geneset))
for(i in paste0('ecosystem',1:6)){#i='ecosystem1'
...
write.table(GSEA_ESscore,'/10X Visium/new_st_NMF/eco_DE_Imm/Imm_GSEA_ESscore_6_hallmark.txt',quote = F,sep = '\t')
write.table(GSEA_NESscore,'/10X Visium/new_st_NMF/eco_DE_Imm/Imm_GSEA_NESscore_6_hallmark.txt',quote = F,sep = '\t')
write.table(GSEA_FDR,'/10X Visium/new_st_NMF/eco_DE_Imm/Imm_GSEA_FDR_6_hallmark.txt',quote = F,sep = '\t')

GSEA_ESscore<-read.delim('/10X Visium/new_st_NMF/eco_DE_Imm/Imm_GSEA_ESscore_6_hallmark.txt')
GSEA_NESscore<-read.delim('/10X Visium/new_st_NMF/eco_DE_Imm/Imm_GSEA_NESscore_6_hallmark.txt')
GSEA_FDR<-read.delim('/10X Visium/new_st_NMF/eco_DE_Imm/Imm_GSEA_FDR_6_hallmark.txt')
...
print(p)
pdf('/10X Visium/new_st_NMF/eco_DE_Imm/00pic/ecosystem_ImmGSEA_6_hallmark.pdf',width = 10,height = 6)
print(p)
dev.off()
